...
% *************************************************
  ...
  %% Open the Simulink Model if it is not open. It has to be in the same folder
open_system(mdl);
  % For the second simulation part
N0_vals = 1:20;
  % For the first part of simulation
G_vals = 1:10;
  % valid only for part 1 of simulation
n = length(G_vals);
E0N0 = zeros(1,n);
Pb_theory = zeros(1,n);
Pb_prac = zeros(1,n);
...
for i = 1:n
      % In the model, the constant block 'Noise' needs to be set to this variable
    N_0 = Simulink.Parameter(10);
      % In the model the value of the block Gain needs to be set to this variable
    G = Simulink.Parameter(G_vals(i));
    ...
    E0N0(i) = E0/N0; % Store the theoretical value of E_0/N_0 to the corresponding array
      % Calculate and store the theoretical value of Pb
    Pb_theory(i) = qfunc(sqrt(2*E0/N0));
      % Get the final value of Pb from the current simulation run from the output struct
